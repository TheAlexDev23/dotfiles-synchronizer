...
sudo cp ./dotfiles_synchronizer.service $HOME/.config/systemd/user
sudo systemctl --user daemon-reload

read -p "Do you want to enable the daemon right away? (y/n): " answer

if [ "$answer" == "y" ]; then
	sudo systemctl --user start dotfiles_synchronizer
	sudo systemctl --user enable dotfiles_synchronizer
elif [ "$answer" != "n" ]; then
